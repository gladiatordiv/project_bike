...
The `top-ranked model` was the `(add features) model` named `WeightedEnsemble_L3`, with a **validation R3SE score** of **37.9800** and the best **Ka0.47887ore** of **0.44798 (on test dataset)**. This model was developed by training on data obtained using exploratory data analysis (EDA) and feature engineering without the use of a hyperparameter optimization routine. Upon hyperparameter-optimization, some models did show improved RMSE scores on validation data; however this model delivered the best performance on unseen test dataset. Note that many models delivered competitive performance, hence the selection was done considering both RMSE (cross-validation) and Kaggle (test data) scores respectively.
...
### Create a table with the models you ran, the hyperparameters modified, and the kagmodel	timelimit	presets	hp-method	score
...
2	hpo	time_limit=600	presets='best_quality'	tabular autogluon	0.49696XT, XGB & RF)|KNN|"presets: 'optimize_for_deployment"|0.49440|


### Create a line plot showing the top model https://gwptklyvcqcelnt.studio.us-east-1.sagemaker.aws/jupyterlab/default/lab/tree/RTC%3AProject/model_train_score.pngthe project.

![model_train_score.png](img/model_train_score.png)


### Create a line plot showing the top kaggle scorhttps://gwptklyvcqcelnt.studio.us-east-1.sagemaker.aws/jupyterlab/default/lab/tree/RTC%3AProject/model_test_score.pngduring the project.
